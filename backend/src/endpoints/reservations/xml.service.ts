...
export class XmlService {
  ...
  prepareXML(reservationData: ReservationDto, isProform: boolean): {} {
    const beallitasok: Beallitasok = {
        szamlaagentkulcs: process.env.INVOICE_KEY, //this.configService.get<string>('INVOICE_KEY'),
        eszamla: true,
      ...
      fejlec: Fejlec = {
        ...
        teljesitesDatum: `${this.year}-${
          this.month < 10 ? "0" + this.month : this.month
        }-${this.day < 10 ? "0" + this.day : this.day}`,
        fizetesiHataridoDatum: `${this.deadLineYear}-${
          this.deadLineMonth < 10
            ? "0" + this.deadLineMonth
            : this.deadLineMonth
        }-${this.deadLineDay < 10 ? "0" + this.deadLineDay : this.deadLineDay}`,
        ...
        dijbekeroSzamlaszam: isProform
          ? ""
          : reservationData.paymentProformNumber,
        elolegszamla: false,
        vegszamla: false,
        helyesbitoszamla: false,
        dijbekero: isProform,
        szamlaszamElotag: process.env.INVOICE_PREFIX, //this.configService.get<string>('INVOICE_PREFIX')
      },
      elado: Elado = {
        bank: process.env.INVOICE_BANK, //this.configService.get<string>('INVOICE_BANK'),
        bankszamlaszam: process.env.INVOICE_ACCOUNT, //this.configService.get<string>('INVOICE_ACCOUNT'),
        emailReplyto: process.env.MAILER_EMAIL, //this.configService.get<string>('MAILER_EMAIL'),
        emailTargy: `Értesítés új ${
          isProform ? "díjbekérő" : "számla"
        } kiállításáról`,
        emailSzoveg: `<p>Új ${
          isProform ? "díjbekérő" : "számla"
        } került kiállításra.</p>
                <p><strong>Vevő:</strong> ${reservationData.name}</p>
                <p><strong>Cím:</strong> ${reservationData.zip} ${
          reservationData.city
        }, ${reservationData.street} ${
          reservationData.addressMisc
            ? reservationData.address + ", " + reservationData.addressMisc
      ...
      tetel: Tetel = {
        ...
        bruttoEgysegar: parseInt(process.env.INVOICE_GROSS, 10), //this.configService.get<number>('INVOICE_GROSS'),
      },
      xml = {
        xmlszamla: {
          beallitasok,
